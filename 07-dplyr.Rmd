...
3-1) 다음은 앞에서 수행했던 `airquality` 데이터를 월별로 나눈 데이터셋임. 이 데이터셋을 이용하여 각 월별로 온도와 오존 농도의 평균값을 구하는 코드를 작성하시오. 단, Ozone과 Temp에서 `NA`를 제외한 데이터셋을 만들고 (`subset` 사용) Ozone, Temp, Month 변수를 선별 (`subset`사용), 그룹별로 데이터를 나눈 후 (`split` 사용) `lapply`, `colMeans` 함수를 사용하여 각 그룹별 평균을 구하시오.

```{r, echo=F}
df <- subset(airquality, !is.na(Ozone) & !is.na(Temp), select=c(Ozone, Temp, Month))
df2 <- split(df, df$Month)
df3 <- lapply(df2, colMeans)
```
